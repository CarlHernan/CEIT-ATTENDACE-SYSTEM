        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div class="card p-4 sm:col-span-2">
                    <p class="text-xs font-semibold uppercase text-blue-800">Year level breakdown</p>
                    <div class="mt-2 flex flex-wrap gap-3 text-sm text-slate-700">
                        @forelse ($summary['by_year'] ?? [] as $year => $count)
                            <span class="inline-flex items-center px-3 py-1 rounded-full bg-slate-100">{{ $year ?: 'N/A' }}: {{ $count }}</span>
                        @empty
            ...
            <div class="card p-5 space-y-4">
                <form method="GET" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div>
                        ...
                        <x-text-input id="section" name="section" type="text" class="mt-1 block w-full" value="{{ request('section') }}" placeholder="e.g., A" />
                    </div>
                    <div>
                        <x-input-label for="year_level" :value="__('Year level')" />
                        <x-text-input id="year_level" name="year_level" type="text" class="mt-1 block w-full" value="{{ request('year_level') }}" placeholder="e.g., 3" />
                    </div>
                    <div>
                    ...
                    <div class="sm:col-span-2 lg:col-span-4 flex items-center gap-3">
                        ...
                        <a href="{{ route('events.report', $event) }}" class="text-sm text-blue-800 hover:text-blue-900">Clear</a>
                    </div>
                </form>
            </div>
            ...
            <div class="card p-5 space-y-3">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-3">
                        <form method="GET" action="{{ route('events.report.export', $event) }}" class="flex items-center gap-2">
                        ...
                        <form method="GET" action="{{ route('events.report.export-absent', $event) }}" class="flex items-center gap-2">
                            <select name="format" class="rounded-lg border-gray-300 text-sm">
                                <option value="xlsx">XLSX</option>
                                <option value="csv">CSV</option>
                            </select>
                ...
                <div class="overflow-auto">
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            @empty
                                <tr><td colspan="10" class="px-3 py-3 text-slate-500">No attendance records.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
            ...
            <div class="card p-5 space-y-3">
                <div class="flex items-center justify-between">
                    <h3 class="text-lg font-semibold text-blue-900">Absent attendees (based on audience)</h3>
                </div>
                <div class="overflow-auto">
                    <table class="min-w-full text-sm">
                        <thead class="bg-slate-50">
