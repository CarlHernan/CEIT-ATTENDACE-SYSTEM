            ...
            <div class="card p-6 card-animate">
                ...
                <div class="mt-6 flex justify-end gap-3">
                    <form method="POST" action="{{ route('events.cancel', $event) }}" class="inline">
                        @csrf
                        @method('PATCH')
                        <button type="submit" class="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white text-sm font-semibold rounded-lg transition" onclick="return confirm('Cancel this event? It will be moved to the cancelled list.')">Cancel Event</button>
                    </form>
                    <form method="POST" action="{{ route('events.destroy', $event) }}" class="inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-semibold rounded-lg transition" onclick="return confirm('Delete this event permanently? This action cannot be undone.')">Delete Event</button>
                    </form>
                </div>
            </div>
        </div>
